...
class StaticHandler(DropboxPathHandler):
    ...
    def check(self, file_meta):
        """
        A fallback handler, so returns true
        """
    	return True

    def __call__(file_meta, request, proxy):
    	"""
    	...
    	headers = {}
    	headers['Content-type'] = file_meta['']
        ...
        for prefix in index_file_prefixes:
    	    for extension in extensions_precedence:
        		...
        		if base_name in index_paths:
        			...
        			new_request_path = request_path + base_name
        			return proxy.handle(new_file_meta, new_request_path)
...
class DBPYHandler(DropboxPathHandler):
    ...
    def __call__(self, request, meta, proxy):
    	...
    	return dbpy.execute.execute(f, request, proxy)
...
class MarkdownHandler(DropboxPathHandler):
    ...
    def __call__(file_meta,request_path,server):
    	...
    	page_body = markdown.markdown(proxy.client.get_file(path).read())
